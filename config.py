...
class Configuration:
    def __init__(self, *args, **kwargs):
        ...
        self.verbose = kwargs.get(
            "verbose", True
        )  # whether to print infections, recoveries and fatalities to the terminal
        self.simulation_steps = kwargs.get(
            "simulation_steps", 10000
        )  # total simulation steps performed
        self.tstep = kwargs.get("tstep", 0)  # current simulation timestep
        self.save_data = kwargs.get(
            "save_data", False
        )  # whether to dump data at end of simulation
        self.save_pop = kwargs.get(
            "save_pop", False
        )  # whether to save population matrix every 'save_pop_freq' timesteps
        self.save_pop_freq = kwargs.get(
            "save_pop_freq", 10
        )  # population data will be saved every 'n' timesteps. Default: 10
        self.save_pop_folder = kwargs.get(
            "save_pop_folder", "pop_data/"
        )  # folder to write population timestep data to
        self.endif_no_infections = kwargs.get(
            "endif_no_infections", True
        )  # whether to stop simulation if no infections remain
        ...
        )  # after this proportion is infected, lock-down begins
        ...
        self.plot_path = kwargs.get(
            "plot_path", "render/"
        )  # folder where plots are saved to
        ...
        self.pop_size = kwargs.get("pop_size", 5000)
        ...
        self.risk_age = kwargs.get(
            "risk_age", 55
        )  # age where mortality risk starts increasing
        self.critical_age = kwargs.get(
            "critical_age", 75
        )  # age at and beyond which mortality risk reaches maximum
        self.critical_mortality_chance = kwargs.get(
            "critical_mortality_chance", 0.1
        )  # maximum mortality risk for older age
        ...
        )  # global baseline chance of dying from the disease
        ...
        )  # when in treatment, affect risk by this factor
        self.no_treatment_factor = kwargs.get(
            "no_treatment_factor", 3
        )  # risk increase factor to use if healthcare system is full
        # risk parameters
        self.treatment_dependent_risk = kwargs.get(
            "treatment_dependent_risk", True
        )  # whether risk is affected by treatment
    ...
    def get_palette(self):
        ...
        palettes = {
            ...
            "deuteranopia": {
                "default": ["gray", "#a50f15", "#08519c", "black"],
